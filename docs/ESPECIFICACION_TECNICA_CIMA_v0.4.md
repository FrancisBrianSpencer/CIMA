...
**Estado:** Fundación operativa; CRUD de residentes y primer corte de ficha personal implementados.  
**Versión de software de referencia:** v0.3 (base publicada); esta revisión técnica describe cambios posteriores todavía sin release/tag.
...
- **0.4, 25-09-2026:** CRUD de residentes completado; incluye consulta individual, edición parcial, archivado lógico, timestamps, acciones de interfaz y pruebas unitarias/de integración.
...
| Residentes | `GET/POST /api/v1/residents`, `GET/PATCH/DELETE /api/v1/residents/{id}`. `DELETE` archiva lógicamente; no elimina físicamente. |
...
| Validaciones API | IDs MongoDB válidos; nombres no vacíos; estado limitado a `active`/`inactive` para creación/edición; DTOs JSON estrictos y límite de 1 MiB. La ficha valida fecha `YYYY-MM-DD` y correo. CORS permite `PATCH` y `DELETE` desde el origen configurado. |
| Timestamps | Creación y actualización registran `createdAt` y `updatedAt`; archivado registra `archivedAt`. Los timestamps faltantes en registros antiguos se omiten en JSON. |
| Interfaz | Crear/listar residentes, consultar ficha, editar nombre/apellido y archivar con confirmación. El registro archivado permanece visible y sus datos se conservan. |
| Verificación ejecutada | Build de ambas imágenes Docker; pruebas unitarias Go; ciclo real POST/GET/PATCH/DELETE contra MongoDB; rechazo HTTP 400 de ID, nombre y propiedad inválidos; preflight CORS para `PATCH`/`DELETE`; healthcheck y frontend HTTP 200. Los registros sintéticos de prueba se eliminaron. |

**Todavía no implementado:** filtros, búsqueda, paginación, autenticación, autorización, habitaciones, alimentación, datos médicos, medicamentos, auditoría, pagos, documentos y dashboards. Hay pruebas unitarias de validación/JSON y verificación manual de integración con MongoDB; falta una suite automatizada de integración API/MongoDB y pruebas E2E.
...
**Implementado:** crear, listar, obtener por ID, actualizar parcialmente y archivar residentes. Campos iniciales: `id`, `firstName`, `lastName`, `status`, `createdAt`, `updatedAt`; al archivar se registran `archivedAt` y `status: "archived"`. MongoDB genera el ID. El backend recorta espacios, rechaza nombres vacíos, limita el estado de creación/edición a `active`/`inactive` y rechaza campos JSON desconocidos.

**Pendiente:**

- Búsqueda, filtros, paginación, ordenamiento y estados definidos.
- Pruebas automatizadas de integración API/MongoDB y E2E. Las pruebas de ciclo CRUD realizadas hasta ahora son manuales.
...
Están implementados `GET /health`, `GET/POST /residents`, `GET/PATCH/DELETE /residents/{id}` y `GET/PATCH /residents/{id}/profile`. Las demás rutas son objetivo, no funcionalidad disponible.
...
- **Unitario:** `go test` para backend y Vitest para frontend; validadores, reglas, servicios y componentes críticos. Hay cobertura Go para estados y decodificación JSON estricta.
- **Integración:** API con MongoDB: creación, lectura individual, actualización, archivado, validaciones y errores. El CRUD se verificó manualmente; automatizar esta suite sigue pendiente.
...
|---:|---|---|
| 1 | Terminar CRUD de residentes | Implementado: creación, listado, lectura individual, edición y baja lógica; falta automatizar pruebas de integración. |
...
| 17 | Testing integral | Parcial: pruebas unitarias Go y verificaciones manuales CRUD/MongoDB; Vitest, integración automatizada y E2E pendientes. |
...
Automatizar las pruebas de integración API/MongoDB para residentes y ficha, y después avanzar con alojamiento (habitaciones/estadías) según las reglas operativas acordadas. Mantener Docker como vía de ejecución y no cargar datos reales hasta disponer de autenticación, autorización y medidas de privacidad adecuadas.
